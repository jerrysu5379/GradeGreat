...
class SQL_Data_Preparer(ChatGPT_Handler):
    ...
    def __init__(self, sql_engine, st, db_path=None, dbserver=None, database=None, db_user=None, db_password=None,
                 **kwargs) -> None:
        super().__init__(extract_patterns=extract_patterns, **kwargs)
        if sql_engine == "sqlserver":
            # TODO: Handle if there is not a driver here
            self.sql_query_tool = SQL_Query(driver='ODBC Driver 17 for SQL Server', dbserver=dbserver,
    ...
    def run(self, question: str, show_code, show_prompt, st) -> str:
        ...
        while count <= max_steps:
            llm_output, next_steps = self.get_next_steps(user_question=user_question, assistant_response=new_input,
                                                         stop=["Observation:"])
            ...
            for output in next_steps:
                ...
                if len(python_code) > 0:
                    ...
                    try:
                        ...
                        new_input += "\nObservation: Encounter following error:" + str(
